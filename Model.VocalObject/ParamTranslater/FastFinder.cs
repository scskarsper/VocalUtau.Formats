...
namespace VocalUtau.Formats.Model.VocalObject.ParamTranslater
{
    public class FastFinder
    {
        ...
        public static int FindNoteTickIndex(long BeFindTick, ref TickSortList<NoteObject> PointList, int LeftBound, int RightBound)
        {
            if (LeftBound > RightBound) return -1;
            int mid = (LeftBound + RightBound) / 2;
            if (LeftBound == mid) return LeftBound;
        ...
        public static int FindNoteTickIn(long BeFindTick, ref TickSortList<NoteObject> PointList, int LeftBound, int RightBound)
        {
            if (LeftBound > RightBound) return -1;
            int mid = (LeftBound + RightBound) / 2;
            if (LeftBound == mid) return LeftBound;
